...
class SeanceRepository extends ServiceEntityRepository
{
    ...
    public function findWithFilters(array $filters = [], string $sortBy = 'startTime', string $sortOrder = 'DESC'): array
    {
        ...
        if (!empty($filters['course']) && $filters['course'] instanceof Course) {
            $qb->andWhere('s.course = :course')
                ->setParameter('course', $filters['course']);
        }

        // Filter by course ID
        if (!empty($filters['courseId'])) {
            $qb->andWhere('s.course = :courseId')
    ...
    /**
     * Get session statistics - THIS WAS MISSING!
     */
    public function getStatistics(): array
    {
        ...
        // Get total sessions
        $totalQuery = $this->createQueryBuilder('s')
            ->select('COUNT(s.id)')
            ->getQuery();
        $total = $totalQuery->getSingleScalarResult();

        // Get upcoming sessions
        $upcomingQuery = $this->createQueryBuilder('s')
            ->select('COUNT(s.id)')
            ->where('s.startTime > :now')
            ->setParameter('now', $now)
            ->getQuery();
        $upcoming = $upcomingQuery->getSingleScalarResult();

        // Get past sessions
        $pastQuery = $this->createQueryBuilder('s')
            ->select('COUNT(s.id)')
            ->where('s.endTime < :now')
            ->setParameter('now', $now)
            ->getQuery();
        $past = $pastQuery->getSingleScalarResult();
