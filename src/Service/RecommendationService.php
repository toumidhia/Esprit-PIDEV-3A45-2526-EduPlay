...
class RecommendationService
{
    ...
    public function getRecommendationsForUser(?User $user = null, int $limit = 5): array
    {
        ...
        if ($user) {
            ...
            if (!empty($userCommandes)) {
                $purchasedProductIds = array_map(fn($c) => $c->getIdProduct()->getId(), $userCommandes);
                $avgPrice = array_sum(array_map(fn($c) => $c->getIdProduct()->getPrice(), $userCommandes)) / count($userCommandes);
    ...
    public function getFrequentlyBoughtTogether(Product $product, int $limit = 3): array
    {
        ...
        $userIds = array_unique(array_map(fn($c) => $c->getIdUser()->getId(), $commandesWithProduct));
